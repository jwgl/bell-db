...
'ROLE_OBSERVATION_ADMIN' AS role_id
FROM tm.observer s
WHERE s.observer_type_id = 0 = 0
UNION ALL
...
CREATE OR REPLACE VIEW tm.dv_observation_count AS
 SELECT teacher.id AS teacher_id,
  ...
  WHERE term.active IS TRUE AND role.name = '校督导'
 ...
 WITH active_term AS (
         ...
         SELECT DISTINCT courseteacher.id AS teacher_id,
          ...
          WHERE courseclass.term_id = (( SELECT active_term.id
                   FROM active_term)) AND schedule.end_week::double precision > (( SELECT date_part('week', now()) - date_part('week', term.start_date) + 1::double precision AS d
                   FROM ea.term
                  WHERE term.active IS TRUE))
        ), new_teacher AS (
         ...
         SELECT DISTINCT inspector.teachercode AS teacher_id
          ...
          WHERE inspector.teachercode IS NOT NULL AND inspector.type = '督导' AND (inspector.term_id + 20) > (( SELECT active_term.id
                   FROM active_term))
        UNION
         SELECT DISTINCT supervisor.teacher_id
           FROM tm.observation_form supervisor
             ...
             JOIN ea.course_class courseclass ON task.course_class_id = courseclass.id
          WHERE role.name = '校督导' AND (courseclass.term_id + 20) > (( SELECT active_term.id
                   FROM active_term))
        )
