...
select course_class.term_id,
  ...
  case when substring(task.code, 24, 5) = task_schedule.teacher_id then 1000 else 2000 end +
  translate(substring(task.code from '-(\d+[A-Z]?)$'), 'ABCDEFG', '1234567')::integer *
  case when course_item.name = '理论' then 10 else 1 end as task_ordinal,
...
and (course_item_workload_settings.course_item_id is null or course_item_workload_settings.type = 1) /*按课程或不计*/;
