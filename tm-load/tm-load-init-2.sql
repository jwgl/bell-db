...
-- 教学形式类别
insert into tm_load.instructional_mode(id, name, ratio, upper_bound) values(10, '理论课', 1.0, 0);
insert into tm_load.instructional_mode(id, name, ratio, upper_bound) values(20, '通识课实验', 0.6, 0);
insert into tm_load.instructional_mode(id, name, ratio, upper_bound) values(21, '专业课实验', 0.8, 0);
insert into tm_load.instructional_mode(id, name, ratio, upper_bound) values(22, '综合性实验', 1.0, 0);
insert into tm_load.instructional_mode(id, name, ratio, upper_bound) values(30, '双语课', 1.2, 0);
insert into tm_load.instructional_mode(id, name, ratio, upper_bound) values(31, '全外语课', 1.5, 0);
...
insert into tm_load.instructional_mode(id, name, ratio, upper_bound) values(50, '毕业论文（设计）', 0.5, 0);
insert into tm_load.instructional_mode(id, name, ratio, upper_bound) values(51, '毕业论文（设计）', 0.7, 0);
insert into tm_load.instructional_mode(id, name, ratio, upper_bound) values(99, '其他', 1.0, 0);
-- 班级规模类别
...
select distinct subject.id, case
  when subject.id like '15%' or discipline.name in ('理学', '工学') then 1
  else 2 end as class_size_type_id
from ea.discipline
...
-- 课程工作量设置
insert into tm_load.course_workload_settings(department_id, course_id, category, parallel_ratio, workload_type, class_size_type_id, instructional_mode_id)
-- 外国语大学外语
select distinct '15', c.id, '大学外语', 0.72, 2 /*课时*/, 5 /*通识课*/, 10 /*理论课*/
...
and term_id >= 20161
union all -- 商学部大学外语
select '20', id, '大学外语', 0.72, 2 /*课时*/, 5 /*通识课*/, 10 /*理论课*/
from ea.course where id like '031100_1'
union all -- 大学体育
select distinct '92', c.id, '大学体育', 0.72, 2 /*课时*/, 5 /*通识课*/, 10 /*理论课*/
from ea.course_class cc
join ea.course c on c.id = cc.course_id
where cc.property_id = 1
...
and term_id >= 20161
union all -- 政治理论课
select '93', id, '政治理论课', null, 2 /*课时*/, 5 /*政治理论课*/, 10 /*理论课*/
from ea.course
where id like '93%'
and id in (
	select course_id
...
union all -- 不计-政治理论课
select '93', course.id, '政治理论课', null, 1 /*不计*/, 9, 10
from ea.course
where name in ('思想道德修养与法律基础（实践）', '形势与政策')
union all -- 毕业论文（设计）
select distinct course_class.department_id, course.id, '毕业论文（设计）', null::numeric(3,2), 4 /*学生*/, 9 /*常量*/,
...
-- 课程项目工作量设置
insert into tm_load.course_item_workload_settings(department_id, course_item_id, category, workload_type, class_size_type_id, instructional_mode_id)
select distinct course_class.department_id, course_item.id, '实验', 2 /*课时*/ , 3 /*实验课*/, 22 /*综合性实验*/
from ea.course_class
...
union all
select distinct course_class.department_id, course_item.id, '体育俱乐部', 1 /*不计*/, 9 /*常量*/, 99 /*其他*/
from ea.course_class
join ea.task on task.course_class_id = course_class.id
join ea.course_item on task.course_item_id = course_item.id
where term_id >= 20161
...
union all
select distinct course_class.department_id, course_item.id, '校体育队', 1 /*不计*/, 9 /*常量*/, 99 /*其他*/
from ea.course_class
join ea.task on task.course_class_id = course_class.id
join ea.course_item on task.course_item_id = course_item.id
where term_id >= 20161
...
union all
select distinct course_class.department_id, course_item.id, '阳光长跑', 1 /*不计*/, 9 /*常量*/, 99 /*其他*/
from ea.course_class
join ea.task on task.course_class_id = course_class.id
join ea.course_item on task.course_item_id = course_item.id
where term_id >= 20161
