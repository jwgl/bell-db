insert into sync_config(id, schema, basic_table, foreign_table) values 
('ea.term',                 'ea', 'term',                 'sv_term'),
...
update sync_config
   set before_sync = $$insert into ea.sv_course_class_map values(null);$$,
       upsert_condition = $$term_id = ${term_id}$$,
       delete_condition = $$term_id = ${term_id}$$
 where id = 'ea.course_class';

update sync_config
   set upsert_condition = $$term_id = ${term_id}$$,
...
update sync_config
   set before_sync = $$insert into ea.sv_task_map values(null);$$,
       upsert_condition = $$term_id = ${term_id}$$,
       delete_condition = $$course_class_id in (
    select id
    from ea.course_class
...
update sync_config
   set select_sql = $$with task_schedule as (
...
select id, day_of_week, end_week, odd_even, place_id, min(start_section) as start_section,
     start_week, task_id, teacher_id, sum(total_section) as total_section
from formal
...
update sync_config
   set select_sql = $$select u1.id, u1.department_id, coalesce(u2.email, u1.email) as email,
